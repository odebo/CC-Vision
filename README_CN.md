...
> 📹 这里应该有一个真实的终端录制 GIF（`docs/demo.gif`）。欢迎提 PR — 见 [`docs/demo.md`](docs/demo.md)。

**Before**（非多模态主力模型，未装 hook）：

```
You: [粘贴截图] 这段代码哪里错了？
AI: 我看到您分享了一张图片，但我无法直接查看图片。
    您能描述一下截图内容，或者把代码贴成文字吗？
```

**After**（同样主力模型，装了 CC-Vision）：

```
You: [粘贴截图] 这段代码哪里错了？
[状态] 解析图片中...
AI: 看截图，第 42 行有错——`foo` 未定义。你 import 的是 `fool`，
    调用的却是 `foo`。改 `foo()` 为 `fool()`，或修正 import。
```

主模型从未看到像素——它看到的是通过 `additionalContext` 注入的文字描述。详见 [`docs/demo.md`](docs/demo.md)，欢迎贡献真实 GIF。
...
一篇公众号文章，复盘了这个问题的发现到解决过程，正在筹备发布。发布后链接会补充在这里。
...
- 多图并发解析（`concurrent.futures`）
- 更多 provider 预设（火山豆包、百度千帆等）
- 内容哈希缓存（跨 session 跳过重复图）
- 一个真实的演示 GIF（见 [`docs/demo.md`](docs/demo.md)）
- 测试（目前由 [CI workflow](.github/workflows/ci.yml) 通过 `--help` 和退出码验证；真正的单元测试更好）

请保持脚本零依赖（仅标准库），这样安装只需一次 `cp`。
